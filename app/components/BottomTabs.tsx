...
export default function BottomTabs() {
  ...
  const tabs = [...NAV_ITEMS.filter(t => t.id !== '/acquisitions'), { id: '/profile', match: (p: string) => p.startsWith('/profile') || p.startsWith('/settings') || p.startsWith('/acquisitions') || p.startsWith('/inbox'), icon: User, label: 'You' }];
          ...
          return (
            <Link key={tab.id} href={tab.id} className="relative flex flex-col items-center justify-center gap-1">
              <span className={`relative flex items-center justify-center w-11 h-7 rounded-full transition-colors ${active ? 'bg-[var(--accent)]/15 text-[var(--accent)]' : 'text-zinc-500'}`}>
                ...
                {tab.id === '/profile' && unreadInbox > 0 && <span className="absolute -top-0.5 right-1.5 w-2 h-2 rounded-full bg-[var(--accent)]" />}
              </span>
